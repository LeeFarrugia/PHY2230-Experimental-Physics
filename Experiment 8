...
# defining minimum readbility and actual values
dA = 1/120
dB = 1/120
A = 1.5220
B = 4.59e-15
...
print(f'A is found to be: {coeffs[1]:.2f} with an error of: {aerr:.2e}')
print(f'A has an accuracy of: {accuracyA:.2f}% and a precision of: {precisionA:.2f}%')
print(f'B is found to be: {coeffs[0]:.2e} with an error of: {berr:.2e}')
print(f'B had an accuracy of: {accuracyB:.2f}% and a precision of: {precisionB:.2f}%')
...
# plotting the figure
plt.errorbar(X, Y, xerr=0, yerr=deltaY, fmt='o', color='k', elinewidth=2, capthick=2, capsize=5,
